...
const ProjectCard: React.FC<ProjectCardProps> = ({ project }) => {
  const handleImageError = (e: React.SyntheticEvent<HTMLImageElement>) => {
    ...
    if (parent) {
      parent.classList.add('bg-gradient-to-br', 'from-brand-primary-100', 'to-blue-100');
      const fallback = document.createElement('div');
      fallback.className = 'flex flex-col items-center justify-center h-full text-brand-primary-700';
      fallback.innerHTML = `
        <div class="w-16 h-16 bg-brand-primary-200 rounded-full flex items-center justify-center mb-4">
  ...
  return (
    <Link href={`/portfolio/${project.slug}`} legacyBehavior>
      <a className="card group block overflow-hidden transform transition-all duration-300 hover:shadow-2xl hover:-translate-y-1">
        ...
        <div className="p-6">
          <h3 className="text-xl font-bold text-slate-800 mb-2 group-hover:text-brand-primary-700 transition-colors duration-200 line-clamp-2">
            {project.title}
          </h3>
          {project.description && (
            <HTMLContent 
          ...
          <div className="flex items-center justify-end text-sm font-semibold text-brand-primary-600 group-hover:text-brand-primary-700 transition-colors duration-200">
            Detayları Gör
