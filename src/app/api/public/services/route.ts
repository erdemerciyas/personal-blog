...
export const GET = asyncHandler(async (req: Request) => {
  ...
  logger.apiRequest('GET', '/api/public/services');
  ...
  logger.apiResponse('GET', '/api/public/services', 200, duration);
...
export const POST = withSecurity(SecurityConfigs.admin)(asyncHandler(async (request: Request) => {
  const startTime = Date.now();
  logger.apiRequest('POST', '/api/public/services');
  ...
  try {
    ...
    const duration = Date.now() - startTime;
    logger.apiResponse('POST', '/api/public/services', 201, duration);
