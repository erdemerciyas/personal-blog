  ...
  try {
    ...
    return {
      ...
      openGraph: {
        ...
        description: translation.metaDescription,
        type: 'article',
        url: `https://fixral.com/es/noticias/${news.slug}`,
        images: [
          {
    ...
    return {
      ...
      description: 'Lee el último artículo de noticias',
    };
  }
}

/**
 * Generate static params for news articles (Spanish)
 */
export async function generateStaticParams() {
  try {
    ...
    logger.error('Error generating static params for news (ES)', 'NEWS_DETAIL', { error });
    return [];
  }
}
...
export default async function NewsDetailPage({ params }: PageProps) {
  try {
    ...
    const jsonLd = {
      ...
      mainEntityOfPage: {
        '@type': 'WebPage',
        '@id': `https://fixral.com/es/noticias/${news.slug}`,
      },
    };
      ...
      <div className="min-h-screen bg-slate-50">
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-6 lg:px-8 py-8">
          ...
          {news.featuredImage?.url && (
            <div className="relative w-full aspect-video mb-8 rounded-2xl overflow-hidden shadow-sm border border-gray-100">
              <Image
          ...
          <div className="grid grid-cols-1 lg:grid-cols-10 gap-6">
              ...
              {news.relatedPortfolioIds && news.relatedPortfolioIds.length > 0 && (
                <section>
                  <div className="flex items-center gap-2 mb-4">
                    <h2 className="text-2xl font-bold text-slate-800">Proyectos Relacionados</h2>
            ...
            <aside className="lg:col-span-3 space-y-6">
              ...
              {news.tags && news.tags.length > 0 && (
                <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                  <h3 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-4">Temas</h3>
              ...
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 text-center">
                ...
                <div className="flex justify-center gap-4">
                  <a
                    href={`https://twitter.com/intent/tweet?url=https://fixral.com/es/noticias/${news.slug}&text=${encodeURIComponent(translation.title)}`}
                    target="_blank"
                  ...
                  <a
                    href={`https://www.facebook.com/sharer/sharer.php?u=https://fixral.com/es/noticias/${news.slug}`}
                    target="_blank"
                    rel="noopener noreferrer"
                  ...
                  <a
                    href={`https://www.linkedin.com/sharing/share-offsite/?url=https://fixral.com/es/noticias/${news.slug}`}
                    target="_blank"
                    rel="noopener noreferrer"
                      ...
                      <Link key={relatedNews._id} href={`/es/noticias/${relatedNews.slug}`} className="flex gap-3 group">
                        ...
                        <div>
                          <h4 className="text-sm font-medium text-slate-900 group-hover:text-fixral-primary line-clamp-2 leading-snug">
                            {relatedNews.translations.es.title}
                          </h4>
                          <div className="text-xs text-slate-500 mt-1">Leer más →</div>
                        </div>
                      </Link>
                    ))}
                  </div>
    ...
    logger.error('Error rendering news detail page (ES)', 'NEWS_DETAIL', { error });
