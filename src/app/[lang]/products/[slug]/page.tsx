...
async function getRelatedProducts(categorySlug?: string, currentSlug?: string) {
  ...
  const base = process.env.NEXTAUTH_URL || '';
  const res = await fetch(`${base}/api/public/products?categorySlug=${encodeURIComponent(categorySlug)}&limit=4`, { next: { revalidate: 60 } });
...
export default async function ProductDetail({ params }: { params: { slug: string } }) {
  ...
  let allCats: any[] = [];
  try {
    const cRes = await fetch(`${baseUrl}/api/public/product-categories`, { next: { revalidate: 300 } });
    if (cRes.ok) {
