...
async function getData(searchParams: Record<string, string>) {
  ...
  try {
    const res = await fetch(`${base}/api/public/products?${qs}`, { next: { revalidate: 60 } });
    if (!res.ok) {
...
export default async function ProductsPage({ params: paramsPromise, searchParams }: { params: Promise<{ lang: string }>; searchParams: Record<string, string> }) {
  ...
  try {
    const catRes = await fetch(`${process.env.NEXTAUTH_URL || 'http://localhost:3000'}/api/public/product-categories`, { next: { revalidate: 300 } });
    if (catRes.ok) {
