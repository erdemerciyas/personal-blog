...
export default function AccountPage() {
    ...
    const handleBulkDeleteMessages = async () => {
        ...
        try {
            ...
            const promises = Array.from(selectedMessages).map(id =>
                fetch(`/api/public/user/messages/${id}`, { method: 'DELETE' })
            );
    ...
    const fetchAddresses = async () => {
        try {
            const res = await fetch('/api/public/user/addresses');
    ...
    const fetchMessages = async (page = 1) => {
        ...
        try {
            const res = await fetch(`/api/public/user/messages?page=${page}&limit=20`);
            const data = await res.json();
            if (data.success) {
    ...
    useEffect(() => {
        if (status === 'authenticated') {
            fetch('/api/public/user/messages?page=1&limit=1').then(res => res.json()).then(data => {
    ...
    const handleSaveAddress = async (e: React.FormEvent) => {
        ...
        setActionLoading(true);
        try {
            const url = editingAddress ? `/api/public/user/addresses/${editingAddress._id}` : '/api/public/user/addresses';
    ...
    const handleDeleteAddress = async (id: string) => {
        ...
        setActionLoading(true);
        try {
            const res = await fetch(`/api/public/user/addresses/${id}`, { method: 'DELETE' });
            const data = await res.json();
            if (data.success) {
    ...
    const handleSetPrimaryAddress = async (id: string) => {
        setActionLoading(true);
        try {
            const res = await fetch(`/api/public/user/addresses/${id}`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
    ...
    const fetchOrders = async (page = 1) => {
        ...
        try {
            const res = await fetch(`/api/public/orders/my-orders?page=${page}&limit=20`);
            const data = await res.json();
            if (data.success) {
                setOrders(data.data);
                if (data.pagination) {
    ...
    const handleCancelOrder = async () => {
        ...
        setActionLoading(true);
        try {
            const res = await fetch(`/api/public/orders/${selectedOrder._id}/cancel`, { method: 'POST' });
            const data = await res.json();
            if (data.success) {
    ...
    const handleDeleteOrder = async (orderId: string) => {
        ...
        setActionLoading(true);
        try {
            const res = await fetch(`/api/public/orders/${orderId}`, { method: 'DELETE' });
            const data = await res.json();
            if (data.success) {
    ...
    const handleSendMessage = async (e: React.FormEvent) => {
        ...
        setActionLoading(true);
        try {
            const res = await fetch('/api/public/orders/messages', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
